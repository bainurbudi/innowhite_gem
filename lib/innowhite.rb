...
class Innowhite
  ...
  def create_room(parent_org,org_name1,user,desc = nil,tags=nil)
    parent_org ||= @parent_org
    org_name1 ||= @org_name 

    room_id = set_room_id
    address = join_room_helper(@server_address,@org_name, room_id, user,true)
    create_room_info(room_id,user,tags,desc) if desc or tags
    return {:address => address, :room_id => room_id}
  end

  def create_room_info(room_id,user,tags,desc)
    RestClient.post("http://innowhite.com/api/create_room_info", {:roomId => room_id, :user => user, :tags => tags,:desc => desc})
  end
